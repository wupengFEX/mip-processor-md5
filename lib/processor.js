...
Processor.prototype.handleFile = function () {
	...
	self.config.paths.forEach(function (pth) {
		var file = path.resolve(self.config.baseUrl, pth);
		self.traverseDir(file, function (fullPath) {
			...
			var outputPath = item ? path.resolve(self.config.outputDir, item[1]) : '';
			var dirname = path.dirname(outputPath);
			var fileName = fobj.base;
			var content = fs.readFileSync(fullPath, CHARSET);
			if (match) {
				var md5 = self.md5(fs.readFileSync(fullPath, CHARSET));
			...
			// 生成map文件
			var relPath = path.relative(self.config.baseUrl, fullPath);
			var outRelPath = relPath.replace(path.basename(relPath), fileName);
			var mapPath = [];
			if (self.config.domain) {
				mapPath.push(self.config.domain);
			}
			mapPath.push(path.basename(self.config.outputDir));
			mapPath.push(outRelPath);
			self.map[relPath] = mapPath.join('/');
		});
	});
	var output = JSON.stringify(self.map, null, 4);
	fs.writeFileSync(path.resolve(self.config.baseUrl, 'mip-conf.json'), output);
}
	...
	self.config.paths.forEach(function (pth) {
		...
		self.traverseDir(file, function (fullPath) {
			...
			if (fullPath.match(/\.css|less|sass$/)) {
				reg = cssReg;
			} else if (fullPath.match(/\.mustache$/)) {
				reg = htmlReg;
			}
			...
			while ((results = reg.exec(content)) != null)  {
				if (results && results.length > 0) {
	                ...
	                var relPath = path.relative(self.config.baseUrl, filePath);
			...
			if (needWrite) {
				var rel = path.relative(self.config.baseUrl, fullPath);
